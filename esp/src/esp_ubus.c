...
char *handle_request(uint32_t pin, char *action, char *port);
				   ...
				   const char *method, struct blob_attr *msg)
{
	...
	response = handle_request(pin, ACTION_ON, obj->name);
					...
					struct blob_attr *msg)
{
	...
	response = handle_request(pin, ACTION_OFF, obj->name);
	if(response != NULL){
		blob_buf_init(&b, 0);
		blobmsg_add_string(&b, "status", response);
		ubus_send_reply(ctx, req, b.head);
